...
const SAVE_USERS_IN_GLOBAL_STATE = 'SAVE_USERS_IN_GLOBAL_STATE';
...
const save_users_in_global_state = () => ({type: SAVE_USERS_IN_GLOBAL_STATE});
...
export const getRandomUsers = () => {
    return async function(dispatch) {
        ...
        if (response.ok) {
            ...
            // save to state
            dispatch(save_users_in_global_state());
        } else {
...
export default function usersReducer(state = INITIAL_STATE, action) {
    switch(action.type) {
        case SAVE_USERS_IN_GLOBAL_STATE:
            return ({...state,
